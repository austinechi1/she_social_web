...
exports.handler=async event=>{
  try{
    ...
    if(method==='POST'){
      ...
      const row={name:b.name.trim(),slug:safeSlug(b.slug||b.name),github_repo:b.github_repo.trim(),github_branch:(b.github_branch||'main').trim(),live_url:(b.live_url||'').trim()||null,pages:Array.isArray(b.pages)&&b.pages.length?b.pages:[{path:'index.html',name:'Home'}],created_by:user.email,updated_at:new Date().toISOString()};
    ...
    if(method==='PATCH'){
      ...
      await owned(b.id);
      const allowed={};
      for(const k of ['name','github_repo','github_branch','live_url','pages']) if(k in b) allowed[k]=b[k];
